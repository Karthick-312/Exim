...
def EBRCUplaod(driver,file_path=r"D:\Testing Plan for Web Application.pdf"):
    #CLick the Insurance Policy menu
    safe_click(driver,"(//h6[normalize-space()='EBRC'])[1]")
...
def EBRCdata(driver):
    ...
    #CLick and enter the EBRC Number
    safe_click(driver,"(//input[@type='text'])[3]")
    safe_send_keys(driver,(By.XPATH,"(//input[@type='text'])[3]"),"54353EER33")
    #CLick the submit button
    safe_click(driver,"(//button[@id='exportrealisationcertificate'])[1]")
...
def test_pass(driver):
    output_path = r"D:\New Python\Recorded Videos\EBRC.mp4"
    ...
    try:
        EBRCUplaod(driver, file_path=r"D:\New Python\Shipping_bill.pdf")
        EBRCdata(driver)
    finally:
        stop_event.set()
...
def test_fail(driver):
    EBRCUplaod(driver,file_path=r"D:\Downloads\2020_Taiwan_Tourism_Guide_Book_(E).pdf")
