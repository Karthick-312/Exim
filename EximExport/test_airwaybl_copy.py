...
def AirwayBillUpload(driver,file_path=r"D:\Testing Plan for Web Application.pdf"):
...
def AirwayBilldata(driver):
    ...
    #CLick and enter Airway/Blcopy number
    safe_click(driver,"(//input[@type='text'])[4]")
    safe_send_keys(driver,(By.XPATH,"(//input[@type='text'])[4]"),"87243633")
    #to submit the airway bill copy
    safe_click(driver,"(//button[@id='AirwayBlCopy'])[1]")
    # to pint the message
...
def test_pass(driver):
    output_path = r"D:\New Python\Recorded Videos\AirWayBillCopy.mp4"
    ...
    try:
        AirwayBillUpload(driver, file_path=r"D:\New Python\samplePDFFile.pdf")
        AirwayBilldata(driver)
    finally:
        stop_event.set()
...
def test_fail(driver):
    AirwayBillUpload(driver,file_path=r"D:\Downloads\pkpadmin,+1008-4741-1-CE (1).pdf")
    AirwayBilldata(driver)
