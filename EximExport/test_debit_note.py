...
def Debitnoteupload(driver,file_path=r"D:\Testing Plan for Web Application.pdf"):
...
def Debitnotedata(driver):
    #Click the Commercial invoice dropdown
    ...
    #Click and enter the credit note number
    safe_click(driver,"(//input[@type='text'])[4]")
    safe_send_keys(driver,(By.XPATH,"(//input[@type='text'])[4]"),"453455")
    #Click and enter the Credit note amount
    safe_click(driver,"(//input[@type='textbox'])[1]")
    safe_send_keys(driver,(By.XPATH,"(//input[@type='textbox'])[1]"),"500")
    # Click teh submit button
    safe_click(driver,"(//button[@id='DebitNoteExport'])[1]")
    #To identify the message
    toast_message = get_toast_alert_text(driver)
    print("message:" + toast_message)

def test_pass(driver):
    output_path = r"D:\New Python\Recorded Videos\DebitNotes.mp4"
    ...
    try:
        Debitnoteupload(driver, file_path=r"D:\New Python\CommercialInvoices_R16846.pdf")
        Debitnotedata(driver)
    finally:
        stop_event.set()
...
def test_fail(driver):
    Debitnoteupload(driver,file_path=r"D:\Downloads\visitor-guide.pdf")
