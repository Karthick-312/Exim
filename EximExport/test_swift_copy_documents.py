...
def SwiftCopyUpload(driver,file_path=r"D:\Testing Plan for Web Application.pdf"):
...
def SwiftCopydata(driver):
    #CLick and enter the swift copy number
    safe_click(driver,"(//input[@type='text'])[3]")
    safe_send_keys(driver,(By.XPATH,"(//input[@type='text'])[3]"),"TY646RT65")
    #CLick the submit button
    safe_click(driver,"(//button[@id='exportswiftcopydocument'])[1]")
...
def test_pass(driver):
    output_path = r"D:\New Python\Recorded Videos\SwiftCopyDocuments.mp4"
    ...
    try:
        SwiftCopyUpload(driver, file_path=r"D:\New Python\Shipping_bill.pdf")
        SwiftCopydata(driver)
    finally:
        stop_event.set()
...
def test_fail(driver):
    SwiftCopyUpload(driver,file_path=r"D:\Downloads\2020_Taiwan_Tourism_Guide_Book_(E).pdf")
